  ...
  const onKeyPress = useCallback(async (key) => {
    ...
    if (key === 'ENTER') {
      ...
      try {
        ...
        const data = await res.json();
        const newGuesses = [...guesses, { word: currentGuess, green: data.green, yellow: data.yellow }];
      ...
      {showRules && (
         <div className="modal-overlay" onClick={() => setShowRules(false)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
               <h2>How to Play</h2>
               <ul>
                  <li>Guess the <b>4-letter word</b>.</li>
                  <li>Every letter must be <b>distinct</b> (no repeated letters).</li>
                  <li>After each guess, you will receive two numbers:</li>
               </ul>
               <div className="rules-example">
                  <div><span className="score green mini">2</span> : Two letters are correct AND in the correct position.</div>
                  <div><span className="score yellow mini">1</span> : One letter is correct BUT in the wrong position.</div>
               </div>
               <p>If you get a 0 and 0, none of those letters are in the secret word! You will be blocked from using them again.</p>
               <button className="close-btn" onClick={() => setShowRules(false)}>Got It</button>
            </div>
         </div>
      )}
              ...
              <div key={i} className="row-wrapper">
                <div className={rowClass}>
                  {Array.from({ length: 4 }).map((_, j) => (
                    <div key={j} className={`tile ${guessObj ? 'filled' : ''} ${isCurrentRow && rowWord[j] ? 'active' : ''}`}>
                      {rowWord[j] || ''}
                    </div>
                  ))}
                </div>
